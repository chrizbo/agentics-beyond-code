...
| [📢 Slack Report-Back](.github/workflows/slack-report-back-dispatch.yml) | After any reporting workflow completes (Weekly Status, Launch Readiness, Workflow Health, etc.), posts a short link to the generated artifact in the configured Slack channel — keeping Slack-native teammates in the loop without requiring them to watch GitHub | Configure `SLACK_ARTIFACT_CHANNEL_MAP` to enable |
