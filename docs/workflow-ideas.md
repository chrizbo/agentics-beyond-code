...
| 23 | ~~Customer Feedback Aggregator~~ | ~~Multi-source feedback monitor: pulls from issues, discussions, Discord, Slack, and social into normalized feedback issues in one repo. Enables dedup, trend detection, and cross-channel prioritization.~~ _Implemented as **[Customer Feedback Intake](../.github/workflows/customer-feedback-intake.yml)** + **[Feedback Dedupe & Strategy Triage](../.github/workflows/feedback-dedupe-triage.md)** — normalizes OSS repo, Discord, and Slack fixtures into the Customer Feedback Queue, then clusters duplicates and triages against strategy. See the full [Cross-Channel Customer Feedback Workflow Spec](cross-channel-customer-feedback-workflow-spec.md)._ |
| 24 | ~~Feedback → Work Item Converter~~ | ~~Slash command on a customer feedback issue that strips PII and customer-specific details, then creates a clean, agent-ready engineering issue. Bridges the PM interpretation gap.~~ _Implemented as **[Feedback Work Item Converter](../.github/workflows/create-work-item.md)** — comment `/create-work-item` on a PM-approved feedback issue to convert it, redacting customer identity while preserving exact language._ |
