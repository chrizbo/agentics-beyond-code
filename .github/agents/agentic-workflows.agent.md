...
- **Integrity filtering (`min-integrity`)**: Set `min-integrity` and `allowed-repos` together on the `tools.github` block to control which content reaches the agent based on author trust level. This is the primary defense against prompt injection from untrusted contributors. `approved` = owners/members/collaborators only; `unapproved` = also includes contributors; `none` = all content (only for fully internal repos). Public repos get `min-integrity: approved` automatically; private repos do not. Always pair `roles: all` with an explicit `min-integrity` setting. See https://github.github.com/gh-aw/reference/github-tools/#guard-policies for details.
